...
const createCard = (data, search) => {
    ...
    if (search == 1) {
        ...
        wpUser = `${data.User.fname} ${data.User.lname}`;
        wpEmail = data.User.email;
        wpMobile = data.User.mobile;
    } else {
        wpName = data.name;
        wpRatings = data.ratings;
        wpDescription = data.description;
    ...
    const deleteButtonHtml = role === "owner"
        ...
        : '';
    const contactButton = role === "owner" ? '' : `<button class="btn btn-primary btn-sm float-end ownerInfo" data-bs-toggle="modal" data-bs-target="#ownerInfo">Contact Owner</button>`;
    const cardContent =
        `
    <div class="card">
    ...
    <div class="card-body">
        ...
        <div class="card-text">
            Price: $ ${data.Leases[0].price} <br>
            Lease Term: ${data.Leases[0].lease_term === null ? 'No Term Chosen' : `${data.Leases[0].lease_term}`}
        </div>
...
searchButton.addEventListener('click', async () => {
    ...
    fetch(`/api/search/workspace/${searchTerm}`)
...
<div class="modal fade" id="${data.id}modal">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                <img src="${data.photos == "/img/" || data.photos == "" ? `/img/coworking.jpg` : data.photos}" alt="Image" style="display: block; margin: 0 auto;">

                    <hr>

                    <span>${data.Property.address === undefined ? 'No Address Provided' : `${data.Property.address}`}</span>
                    <div class="card-text">Capacity: ${data.capacity} </div>
                    <div>
                    ...
                    <div class="card-text">Price: $ ${data.Leases[0].price} <br>Lease Term: ${data.Leases[0].lease_term}
                    <br> Lease Id : ${data.Leases[0].id}
                    </div>
                ...
                ${role === 'owner' ? '' :
            data.availability ?
                `<button class="btn btn-primary book-workspace" data-workspace-id="${data.id}" data-property-id="${parseInt(data.property_id)}">Book</button>` :
